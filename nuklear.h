...
nk_input_key(struct nk_context *ctx, enum nk_keys key, int down)
{
    ...
    NK_ASSERT(ctx);
	if (!ctx) {
		printf("Ctx doesn't exist..");	
		return;
	}else{
		printf("Ctx okay. Continuing..");
	}
    in = &ctx->input;
    ...
    const struct nk_input *i, enum nk_button_behavior behavior)
{
	...
	//TODO: Added by Varuna Singh
	/*
	if (is_this_supposed_to_hover(r)){
		*state = NK_WIDGET_STATE_HOVERED;
	}
	*/
	if (behavior == NK_BUTTON_HOVER){
		*state = NK_WIDGET_STATE_HOVERED;
	}
...
/* Sets background, queues nk drawing commands needed to draw the button */ 
...
/**
	Checks and sets all style related things to button [nk_button_behaviour]
**/
NK_INTERN int
...
/**
	Set things needed to draw button text..
**/
NK_INTERN void
...
/**
	Set things needed to draw button text..
	Checks the style if OK
	Calculates button content space [nk_do_button]
	Set things needed to draw the button text [nk_draw_button_text]
**/
NK_INTERN int
    ...
    const struct nk_user_font *font)
{
	...
	/* Asserts style, state, out, button spacing and button behavior */ 
    ...
    const char *name, const char *title, struct nk_rect bounds, nk_flags flags)
{
    ...
    if (!(win->flags & NK_WINDOW_SUB) && !(win->flags & NK_WINDOW_HIDDEN))
    {
        ...
        float h = ctx->style.font->height + 2 * style->window.header.padding.y; //bug4 is here..
...
/**
	Declares a button in MRE View. 
	This method checks if this widget(button) is possible or not
	If it is supposed to be active on the screen or not (someone
	might have scrolled away from it)
	This method gets the bounds of the button (coordinates)
	Continues with button declaration..
...
nk_button_text(struct nk_context *ctx, const char *title, int len)
{
    ...
    style = &ctx->style;
    layout = win->layout;
	/* Checks if widget is possible.. */
    state = nk_widget(&bounds, ctx);

	/* To not draw, state will be NK_WIDGET_INVALID */
	if (!state){
		return 0;
	}

	/* We now have bounds of the button. 
		A Button is a hoverable and clickable object 
		Add bounds to hoverable list */
	//add_this_to_hoverable_list(bounds);
...
/**
	A Labeled Button is basically a Button with Text in it..
**/
