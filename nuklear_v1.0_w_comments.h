...
#define NK_IMPLEMENTATION
	...
	//typedef int nk__check_rune[(sizeof(nk_rune) >= 4) ? 1 : -1];	//temperary letting it pass
	typedef int nk__check_ushort[(sizeof(nk_ushort) == 2) ? 1 : -1];
	typedef int nk__check_short[(sizeof(nk_short) == 2) ? 1 : -1];
	//typedef int nk__check_uint[(sizeof(nk_uint) == 4) ? 1 : -1];	//temperary letting it pass
	//typedef int nk__check_int[(sizeof(nk_int) == 4) ? 1 : -1];	//temparary letting it pass
	...
	*/
	...
	/*
	Memory Set
	Params are a pointer to an object, intger size of the object(unsigned long)
	nk_byte : unsigned char
	nk_size : unsigned long
	Returns nothing
	*/
	...
	{
		#define nk_word unsigned int
		#define nk_wsize sizeof(nk_word)
		#define nk_wmask (nk_wsize - 1)
		//Here is where the error is. Variable dst is a bad pointer
		//ERROR: 0xC0000005: Access violation writing location 0xfffffef8.
		...
		do {
			...
			nk_word: int
			*(int*)((void*)dst) = c;
			Also: int* i and int *i are completely equivalent 
			Which is initialising a pointer i .
			In this case, we are initialising a pointer 
			0xC0000005: Access violation writing location 0xfffffef8.
			Possible reasons: The variable dst's memory isn't allocated.
			*/
			*(nk_word*)((void*)dst) = c;
			dst += nk_wsize;
		} while (--t != 0);
	...
	*/
	...
	/*
		Creates a window
		Gets nuklear context as parameter
	...
	*/
	...
	nk_create_window(struct nk_context *ctx)
	{
		...
		/*
			nk_zero
			Check the memory, Set the memory
			Problem 1 here
		*/
